...
import 'package:myflutter_exp1/provider/user_provider.dart';
import 'package:myflutter_exp1/screen/login_screen.dart';
import 'package:myflutter_exp1/screen/movie_screen.dart';
import 'package:myflutter_exp1/screen/nfc_screen.dart';
...
final auth = Provider<String>((ref) => AuthenticationHelper().getUUID());
final usrModel = Provider<UserProvider>((ref) => UserProvider());
final usrProvider = FutureProvider<User>((ref) {
  final usr = ref.watch(usrModel);
  return usr.fetchUser(ref.watch(auth));
});


final movieP2 = Provider<GetDataFromMovieApi>((ref) => GetDataFromMovieApi());
final mProvider = FutureProvider((ref) {
  final mv = ref.watch(movieP2);
  return mv.fetchUserFavMovie(ref.watch(auth));
});
...
class _LandingScreenState extends ConsumerState<LandingScreen> {
  ...
  void _activateListeners() async {
    ...
    fb.child('userProfile/${getUUID}').onValue.listen((event) {
      ...
      // usr2 = User.fromJson(Map<String, dynamic>.from(event.snapshot.value))
      setState(() {
  ...
  void initState() {
    ...
    //_fetchMovie();
    //  _activateListeners();
    tabs = [MovieScreen(), NFCScreen(), ProfileScreen()];
    //init();
  }
  ...
  Widget build(BuildContext context) {
   ...
   // final usrP = ref.watch(usrProvider);
   final movieP = ref.watch(mProvider);
    return movieP.when(
        data: (response) {
            ...
            child: Scaffold(
              ...
              bottomNavigationBar: BottomNavigationBar(
                ...
                items: [
                  ...
                  BottomNavigationBarItem(
                      icon: Icon(Icons.nfc_outlined), label: 'NFC'),
                  BottomNavigationBarItem(
